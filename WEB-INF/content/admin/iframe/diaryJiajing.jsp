...
$(document).ready(function(){
	$("div.pagin").pageNum({
		totalPage : ${diaryJiajingBean.totalPage},
		currentPage : ${diaryJiajingBean.currentPage},
...
<article id="d_shantie">
	...
	<s:if test="diaryJiajingBean.list.size() == 0">
		<p class="noData">加精日志为空！</p>
	</s:if>
	<s:else>
		...
		<table>
			...
			<s:iterator value="diaryJiajingBean.list" var="subject">
			<tr>
				<td><s:date name="time" format="yyyy-MM-dd HH:mm"/></td>
				<td>${operate_id }</td>
				<td><s:if test="type == 3">添加精品</s:if><s:else>取消精品</s:else></td>
				<td>${author_id }</td>
				<td>${title }</td>
			</tr>
			</s:iterator>
